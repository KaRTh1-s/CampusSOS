 ...
 * =========================
 * This service simulates the structured JSON response that will be produced by
 * Amazon Bedrock (Amazon Nova / Titan) in Phase 6.
 *
 * It is isolated here so the frontend can seamlessly swap this mock implementation
 * with real API Gateway calls (POST /analyze) without refactoring UI components.
 */

import { AnalysisResult } from '../types/index';

export async function analyzeIssueMock(description: string, _location?: string): Promise<AnalysisResult> {
  // Simulate network & AI model inference latency (800ms - 1200ms)
  ...
  // 5. Security Incident / Theft / Dispute (HIGH) - Presumption of innocence, objective language
  if (
  ...
  // 8. General Maintenance / Structural
  if (
  ...
  // 9. Fallback General Campus Support
  return {
